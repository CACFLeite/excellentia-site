...
export const metadata: Metadata = {
  ...
  description: 'Conheça a Excellentia: formação docente, governança escolar, evidências e leitura institucional para professores e escolas.'
}
...
const audiences = [
  {
    ...
    description: 'Formações para currículo, processo seletivo, aula teste, rotina profissional e posicionamento docente.',
    href: '/formacoes',
    cta: 'Ver formações',
  },
  {
    ...
    description: 'Formações, certificados, registros e leitura de temas sensíveis para equipes, coordenação e gestão.',
    href: '/escolas',
    ...
    <>
        ...
        <div className="relative mx-auto grid max-w-7xl grid-cols-1 items-end gap-12 px-4 py-20 sm:px-6 md:py-28 lg:grid-cols-[1fr_.78fr] lg:px-8">
          <div className="max-w-3xl">
            ...
            <p className="mt-7 max-w-2xl text-lg leading-8 text-slate-200 md:text-xl">
              A Excellentia aproxima desenvolvimento docente, governança escolar e evidências para decisões mais responsáveis.
            </p>
          </div>
          ...
          <div className="rounded-[2.5rem] border border-white/15 bg-white/[0.08] p-7 shadow-2xl backdrop-blur-md">
            ...
            <p className="mt-5 text-2xl font-black leading-tight text-white">
              Não somos uma biblioteca de cursos soltos. Organizamos formação, evidência e repertório para a rotina escolar.
            </p>
      ...
      <section className="relative overflow-hidden bg-navy py-20 text-white">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="overflow-hidden rounded-[2.75rem] border border-white/10 bg-white/[0.055] shadow-2xl backdrop-blur">
            <div className="grid gap-8 border-b border-white/10 bg-white/[0.035] p-8 md:p-10 lg:grid-cols-[.82fr_1fr] lg:items-end">
              <div>
                <div className="mb-4 text-sm font-black uppercase tracking-[0.24em] text-gold-light">atuação</div>
                <h2 className="text-3xl font-black tracking-[-0.03em] text-white md:text-5xl">Dois públicos. A mesma exigência de rigor.</h2>
              </div>
              <p className="max-w-2xl text-lg leading-8 text-slate-300 lg:justify-self-end">
                Para educadores, formação aplicável. Para instituições, formação, certificados, evidências e leitura de risco no mesmo fluxo.
              </p>
            </div>

            <div className="relative p-5 md:p-8">
              <div className="absolute left-1/2 top-8 hidden h-[calc(100%-4rem)] w-px bg-gradient-to-b from-transparent via-gold-light/50 to-transparent lg:block" />
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-8">
                {audiences.map((audience) => (
                  <Link key={audience.label} href={audience.href} className="group flex min-h-[320px] flex-col rounded-[2rem] border border-white/10 bg-white/[0.055] p-7 text-white transition duration-300 hover:-translate-y-1 hover:border-gold-light/40 hover:bg-white/[0.085] md:p-9">
                    <div className="mb-5 text-sm font-black uppercase tracking-[0.24em] text-gold-light">{audience.label}</div>
                    <h3 className="max-w-xl text-4xl font-black tracking-[-0.04em]">{audience.title}</h3>
                    <p className="mt-6 max-w-xl text-lg leading-8 text-slate-300">{audience.description}</p>
                    <span className="mt-auto inline-flex font-black text-gold-light transition group-hover:translate-x-1">{audience.cta} →</span>
                  </Link>
                ))}
              </div>

              <div className="mt-5 rounded-[1.75rem] border border-gold-light/25 bg-gold-light/10 px-6 py-5 md:px-8">
                <div className="grid grid-cols-1 items-center gap-4 lg:grid-cols-[1fr_auto]">
                  <div>
                    <div className="mb-2 text-xs font-black uppercase tracking-[0.24em] text-gold-light">inteligência educacional</div>
                    <p className="text-xl font-black leading-tight text-white md:text-2xl">Repertório só vale quando orienta a rotina.</p>
                  ...
                  <Link href="/inteligencia-educacional" className="rounded-2xl border border-white/30 px-7 py-3 text-center font-black text-white transition hover:bg-white hover:text-navy">
                    Ler análises
                  </Link>
                </div>
              </div>
      ...
      <section className="relative overflow-hidden bg-navy px-4 py-20 text-white sm:px-6 lg:px-8">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(244,219,118,.10),transparent_30%),linear-gradient(180deg,#0a2749_0%,#071d38_100%)]" />
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-gold/40 to-transparent" />
        <div className="relative mx-auto max-w-7xl overflow-hidden rounded-[2.5rem] border border-white/10 bg-white/[0.06] p-8 shadow-2xl backdrop-blur-md md:p-10">
