...
export async function GET(
  ...
  if (invitation.status !== "accepted") {
    return NextResponse.json(
      {
        error:
          "Antes de iniciar o curso, confirme seus dados no link de acesso.",
      },
      { status: 403 },
    );
  }
    ...
    course: {
      ...
      lessons: course.lessons.map((lesson) => {
        ...
        return {
          ...
          title: lesson.title,
          duration:
            typeof lessonMetadata.duration === "string"
              ? lessonMetadata.duration
              : undefined,
          embedUrl:
            lesson.videoUrl ??
            (typeof lessonMetadata.embedUrl === "string"
              ? lessonMetadata.embedUrl
              : undefined),
          activity: activity
            ? {
                id: activity.id,
