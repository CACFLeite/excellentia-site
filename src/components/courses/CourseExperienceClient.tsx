...
function emphasizeText(text: string) {
  ...
  return parts.map((part, index) => (emphasisTerms.includes(part.toLowerCase()) ? <strong key={`${part}-${index}`}>{part}</strong> : part));
}
        ...
        if (itemMatch) {
          return (
            <p key={index} className="pl-3">
              <span className="font-bold text-navy">{itemMatch[1]}</span>
              {emphasizeText(itemMatch[2])}
            </p>
          );
        }
...
type CourseData = {
  ...
  course: { slug: string; title: string; description?: string | null; lessons: Lesson[] };
};

export default function CourseExperienceClient({ token, courseSlug }: { token: string; courseSlug: string }) {
  ...
  async function loadCourse() {
    setLoading(true);
    const response = await fetch(`/api/cursos/${courseSlug}?convite=${encodeURIComponent(token)}`);
  ...
  async function saveAnswer(activityId: string) {
    ...
    const response = await fetch(`/api/cursos/${courseSlug}/respostas`, {
      method: 'POST',
  ...
  async function issueCertificate() {
    ...
    const response = await fetch(`/api/cursos/${courseSlug}/certificado`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  if (error && !data) {
    return (
      <main className="min-h-screen bg-gray-50 px-4 py-12 text-center text-red-700">
        <p>{error}</p>
        <button onClick={() => window.history.back()} className="mt-5 bg-navy hover:bg-blue-950 text-white font-bold px-5 py-3 rounded-xl text-sm">
          Voltar
        </button>
      </main>
    );
    ...
    <main className="min-h-screen bg-gray-50">
      <section className="bg-navy text-white py-12">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <button onClick={() => window.history.back()} className="mb-6 text-sm text-gray-300 hover:text-white">
            ← Voltar
          </button>
          ...
          <p className="text-gray-300 max-w-3xl">
            Olá, {data.employee?.fullName ?? 'colaborador(a)'}. Assista às aulas e responda às situações-problema para formar seu registro de participação.
          </p>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-wrap gap-2">
            {data.course.lessons.map((lesson, index) => (
              <button
                ...
                className={`px-3 py-2 rounded-lg text-sm font-bold ${index === currentLessonIndex ? 'bg-navy text-white' : lesson.activity?.response ? 'bg-green-50 text-green-800' : 'bg-gray-100 text-gray-600'}`}
              >
            ...
            <article key={currentLesson.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              {currentLesson.embedUrl && (
                <div className="aspect-video bg-black">
                  <iframe src={currentLesson.embedUrl} title={`${data.course.title} — ${currentLesson.title}`} className="w-full h-full" allow="accelerometer; gyroscope; autoplay; encrypted-media; picture-in-picture" allowFullScreen />
                </div>
              )}
                  ...
                  {currentLesson.activity.response?.feedback && (
                    <div className="mt-5 rounded-xl bg-green-50 border border-green-100 p-5">
                      ...
                      <ul className="text-sm text-green-900 list-disc pl-5 space-y-1">
                        {currentLesson.activity.response.feedback.nextSteps.map((item) => (
                          <li key={item}>{item}</li>
                        ))}
                      </ul>
                    </div>
              ...
              <div className="p-5 md:p-6 border-t border-gray-100 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <button onClick={() => setCurrentLessonIndex(Math.max(0, currentLessonIndex - 1))} disabled={currentLessonIndex === 0} className="bg-white border border-gray-200 hover:border-gold disabled:opacity-50 text-navy font-bold px-5 py-3 rounded-xl text-sm">
                  Aula anterior
                </button>
                {currentLessonIndex === data.course.lessons.length - 1 ? (
                  data.certificate ? (
                    <a href={`/certificados/${data.certificate.verificationCode}`} className="bg-gold hover:bg-yellow-600 text-white font-bold px-5 py-3 rounded-xl text-sm text-center">
                      Abrir certificado
                    </a>
                  ) : (
                  ...
                  <button onClick={() => setCurrentLessonIndex(Math.min(data.course.lessons.length - 1, currentLessonIndex + 1))} className="bg-navy hover:bg-blue-950 text-white font-bold px-5 py-3 rounded-xl text-sm">
                    Próxima aula
                  </button>
                )}
              </div>
